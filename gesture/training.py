...
def train():
    ...
    with mp_hands.Hands(
        ...
        while cap.isOpened():
            ...
            success, image = cap.read()  # 按帧读取摄像头输出
            # 对图片进行处理
            ...
            # 对每个动作设置单独的置信度阈值
            cfd = {'点击': 0.90, '平移': 0.90, '缩放': 0.99, '抓取': 0.985, '旋转': 0.99, '无': 0, '截图': 0.99,
                   '放大': 0.9}
            if confidence > cfd[movement[rel.item()]]:  # 超过阈值的动作将会被输出
                ...
                if not (now_gesture == last_gesture):  # 判断是否与上次的输出相同，若相同则不输出
                    if time.time() - prin_time > 2:  # 若距离上次输出时间小于2秒，则不输出8
