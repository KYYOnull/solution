...
class Window(QMainWindow, Ui_MainWindow):
    ...
    # 加入会议，若名字不为空则发送用户名，把加入按钮置为已加入会议，并设置为不可用，然后将登录设为TRUE， 把app.identify打开
    def join(self):
    ...
    """获取控制。首先检测登录状态，若已登录则再检测是否正在控制，都先从客户端发送信息‘exchange_control’，若未在控制则置isController为TRUE，设置get_ctrl的文本为退出控制，pause的文本为开始控制，
    ，若已在控制，则置get_ctrl的文本为获取控制，pause为开始控制"""
    def get_ctrl(self):
        if self.isLogin:
